...
def train(epoch, FM_weight):
    ...
    for batch in training_data_loader:
        ...
        # calcuate the semantic loss
        img_f_full_256 = F.interpolate(img_f_full[0], 256)
        g_l1_FM_loss = torch.mean(model.l1_loss_feature(img_f_pred[0], img_f_full_256) * (1 + 3 * mask))
        g_l1_FM_loss = g_l1_FM_loss + torch.mean(model.l1_loss_feature(img_f_pred[1], img_f_full[1]) * (1 + 3 * mask_128))
        g_l1_FM_loss = g_l1_FM_loss + torch.mean(model.l1_loss_feature(img_f_pred[2], img_f_full[2]) * (1 + 3 * mask_64))
