...
if __name__ == '__main__':
    ...
    cat = imresize(imread('test_1.jpg'), (256, 256))
    dog = imresize(imread('test_2.jpg'), (256, 256))
    feats = torch.stack([
    ...
    recon = uncrop_bbox(crops, boxes, 256)
    pdb.set_trace()
    for i in range(recon.size(0)):
        crop_np = recon.data[i].cpu().numpy().transpose(1, 2, 0).astype(np.uint8)
        imsave('out_recon%d.png' % i, crop_np)
